      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-32 left-10 opacity-20">
          <svg width="85" height="115" viewBox="0 0 85 115" fill="none">
            ...
            <ellipse cx="35" cy="52" rx="7" ry="11" fill="#7CB342" transform="rotate(-18 35 52)" opacity="0.5"/>
          </svg>
        </div>
        <div className="absolute bottom-28 right-12 opacity-20">
          <svg width="90" height="120" viewBox="0 0 90 120" fill="none">
            ...
            <ellipse cx="38" cy="55" rx="8" ry="12" fill="#8BC34A" transform="rotate(-22 38 55)" opacity="0.5"/>
          </svg>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-gradient-to-br from-[#F5E6D3] to-[#E8D5B7] rounded-3xl p-8 border-2 border-[#D4A574] shadow-lg"
            >
              <div className={`${feature.color} w-24 h-24 rounded-full flex items-center justify-center mb-5 shadow-md`}>
                {feature.icon}
              </div>
              <h3 className="text-2xl font-bold text-[#8B6F47] mb-4">{feature.title}</h3>
              <p className="text-[#8B6F47] leading-relaxed text-base">{feature.description}</p>
            </div>
          ))}
        </div>
